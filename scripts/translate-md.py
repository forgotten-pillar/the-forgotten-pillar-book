...
Key rules to follow:
1. **Preservation of Markdown Directives:**
    - Ensure that all Markdown directives are preserved in the translated text.
    - Do not add any extra Markdown formatting or commands.
    - Do not remove any Markdown formatting or commands.
    - Do not add any extra commentary or numbering.
...
def translate_paragraph_batch(client, paragraphs, target_lang, system_message):
    delimiter = "===SPLIT==="
    joined_paragraphs = f"\n{delimiter}\n".join(paragraphs)
    prompt = f"""Translate the following paragraphs to {target_lang}. Preserve all Markdown commands, Return exactly {len(paragraphs)} translated paragraphs in the same order as the input, separated by the delimiter '{delimiter}'. The Markdown directives must be preserved fully in its corresponding translation! Do not add any extra commentary or numbering.
...
def process_latex_file(input_file, output_dir, client):
    ...
    system_prompt_config_file = os.path.join(os.path.dirname(output_dir), 'lang', 'ar', "config.yaml")
...
def main():
    ...
    if len(sys.argv) not in [3, 4]:
        sys.exit("Usage: python translate-md.py <target_lang_code> <chapter_file.tex> [overwrite]")
    ...
    output_file = os.path.join(output_dir, "translated.md")
