...
def main():
    # Ensure only 1 argument (target language code) is provided
    if len(sys.argv) != 2:
    ...
    suggestions_dir = os.path.join(root_dir, "lang", target_lang_code, "suggestions")
    config_file = os.path.join(root_dir, "lang", target_lang_code, "config.yaml")

    # Validate essential files/folders
    ...
    if not os.path.isdir(suggestions_dir):
        sys.exit(f"Error: Suggestions directory '{suggestions_dir}' does not exist.")

    # Build the output directory path.
    setup_directories(output_dir)

    # Get all .tex files in input directory
    all_tex_files = sorted([f for f in os.listdir(input_dir) if f.endswith(".tex")])
    total_files = len(all_tex_files)
    if total_files == 0:
        sys.exit("No .tex files found to translate.")

    # Filter out already translated files
    files_to_translate = [
        f for f in all_tex_files
        if not os.path.exists(os.path.join(output_dir, f))
    ]
    translated_count = total_files - len(files_to_translate)
    ...
    # Process each file
    for i, chapter_file in enumerate(files_to_translate, start=1):
        ...
        if not os.path.exists(suggestion_file):
            print(f"Skipping '{chapter_file}': suggestion file not found.")
            continue

        print(f"[{i + translated_count}/{total_files}] Translating '{chapter_file}'...")
        process_latex_file(input_file, output_dir, suggestion_file, client)
        print(f"Completed: '{chapter_file}'")
